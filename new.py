    ...
    with open(csv_file, newline='', encoding='utf-8') as file:
        ...
        for athlete in data[header_index + 1:]:  # Start from the third row (ignoring header rows)
            ...
            athlete_rows += f'''
                <tr>
                    <td>{athlete_place}</td>
                    <td><img class="athleteImages" src="AthleteImages/{athlete_image}" alt="{athlete_name}"></td>  
    ...
    if os.path.isdir(folder_path):
        ...
        for photo in os.listdir(folder_path):
            ...
            if file_ext in photo_extensions:
                counter += 1
                photolist += f'''
                            <a href="{file_path}" target="_blank" data-lightbox="pic" aria-label="Enlarged view of Image #{counter}" title="Enlarged view of Image #{counter}">
                                <img class="photoList" src="{file_path}" alt="Thumbnail of Image #{counter}">
                            </a>'''
